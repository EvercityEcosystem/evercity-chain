...
fn it_denies_add_and_set_roles_for_non_master() {
    new_test_ext().execute_with(|| {
        ...
        <pallet_timestamp::Pallet<TestRuntime>>::set_timestamp(12345);
        assert_noop!(
...
fn it_works_check_events() {
    new_test_ext_with_event().execute_with(|| {
        ...
        assert_eq!(Event::EvercityAccounts(crate::Event::AccountAdd(ROLES[0].0, some_new_account, CC_INVESTOR_ROLE_MASK, 0)),
             add_account_event);
        assert_eq!(Event::EvercityAccounts(crate::Event::AccountSet(ROLES[0].0, some_new_account, CC_AUDITOR_ROLE_MASK)),
             set_account_event);
        // assert_eq!(Event::pallet_evercity_accounts(crate::RawEvent::MasterSet(ROLES[0].0, some_new_account)), set_master_event);
        assert_eq!(Event::EvercityAccounts(crate::Event::AccountWithdraw(ROLES[0].0, some_new_account, CC_AUDITOR_ROLE_MASK)),
