...
fn it_works_for_create_new_file() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Filesign::create_new_file(Origin::signed(owner), tag, filehash, Some(file_id)));
...
fn it_works_for_create_new_file_id_already_exists() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Filesign::create_new_file(Origin::signed(owner), tag.clone(), filehash, Some(file_id)));
...
fn it_works_for_create_new_file_no_file_id() {
	new_test_ext_with_event().execute_with(|| {
		...
		assert_ok!(Filesign::create_new_file(Origin::signed(owner), tag, filehash, None));
...
fn it_fails_for_create_new_file_incorrect_file_input() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Filesign::create_new_file(Origin::signed(owner), tag, filehash, Some(file_id)), Error::<TestRuntime>::EmptyTag);
...
fn it_works_assign_signer() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Filesign::create_new_file(Origin::signed(1), tag, filehash, Some(file_id)));
		assert_ok!(Filesign::assign_signer(Origin::signed(1), file_id, account_id));
		let file_opt = Filesign::get_file_by_id(file_id);
...
fn it_works_delete_signer() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Filesign::delete_signer(Origin::signed(1), file_id, account_id));
...
fn it_fails_delete_signer_no_signers() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Filesign::delete_signer(Origin::signed(1), file_id, account_id),Error::<TestRuntime>::AddressNotSigner);
...
fn it_works_sign_latest_version() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Filesign::assign_signer(Origin::signed(account_id), file_id, account_id));
		assert_ok!(Filesign::sign_latest_version(Origin::signed(account_id), file_id));
...
fn it_fail_sign_latest_version_not_an_signer() {
	new_test_ext().execute_with(|| {
		...
		let _ = Filesign::create_new_file(Origin::signed(account_id), tag, filehash, Some(file_id));
		assert_noop!(Filesign::sign_latest_version(Origin::signed(account_id), file_id),Error::<TestRuntime>::AddressNotSigner);
		let file_opt = Filesign::get_file_by_id(file_id);
