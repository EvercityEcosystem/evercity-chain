...
fn it_works_for_create_new_annual_report_deposit_event_gold_standard() {
    new_test_ext_with_event().execute_with(|| {
        ...
        let check_event = Event::CarbonCredits(crate::Event::AnnualReportCreated(owner, project_id));
...
fn it_works_sign_annual_report_deposit_events_gold_standard() {
    new_test_ext_with_event().execute_with(|| {
        ...
        let tuple_vec = vec![
            (owner, Event::CarbonCredits(crate::Event::AnnualReportSubmited(owner, 1))),
            (auditor, Event::CarbonCredits(crate::Event::AnnualReportSignedByAuditor(auditor, 1))),
            (standard_acc, Event::CarbonCredits(crate::Event::AnnualReportSignedByStandard(standard_acc, 1))),
            (registry, Event::CarbonCredits(crate::Event::AnnualReportSignedByRegistry(registry, 1))),
        ];
