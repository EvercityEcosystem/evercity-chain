...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> where <T as pallet_evercity_assets::pallet::Config>::ABalance: From<u64> + Into<u64> {
        ...
        pub fn create_project(origin: OriginFor<T>, standard: Standard, file_id: Option<FileId>) -> DispatchResultWithPostInfo {
            ...
            ensure!(accounts::Pallet::<T>::account_is_cc_project_owner(&caller), Error::<T>::AccountNotOwner);
        ...
        pub fn create_bond_project(
            ...
            let bond = pallet_evercity_bonds::Pallet::<T>::bond_registry(&bond_id).ok_or(Error::<T>::BondNotFinished)?;
        ...
        pub fn change_project_file_id(origin: OriginFor<T>, project_id: ProjectId, file_id: FileId) -> DispatchResultWithPostInfo {
            let caller = ensure_signed(origin)?;
            ensure!(pallet_evercity_accounts::Pallet::<T>::account_is_cc_project_owner(&caller), Error::<T>::AccountNotOwner);
            ensure!(pallet_evercity_filesign::Pallet::<T>::address_is_owner_for_file(file_id, &caller), Error::<T>::AccountNotFileOwner);
        ...
        pub fn assign_project_signer(
            ...
            ensure!(pallet_evercity_accounts::Pallet::<T>::account_is_selected_role(&signer, role), Error::<T>::AccountIncorrectRole);
            ProjectById::<T>::try_mutate(
                project_id, |project_to_mutate| -> DispatchResultWithPostInfo {
                    match project_to_mutate  {
                        ...
                        Some(project) => {
                            ...
                            pallet_evercity_filesign::Pallet::<T>::assign_signer(origin, file_id, signer.clone())?;
                        }
                    }
                    Ok(().into())
                })?;
                ...
                project_id, |project_to_mutate| -> DispatchResultWithPostInfo {
                    match project_to_mutate  {
                        ...
                        Some(project) => {
                            ...
                            let has_signed = pallet_evercity_filesign::Pallet::<T>::address_has_signed_the_file(file_id, &signer);
                            ensure!(!has_signed, Error::<T>::AccountAlreadySigned);
                            project.remove_required_signer((signer.clone(), role));
                            // delete from filesign:
                            pallet_evercity_filesign::Pallet::<T>::delete_signer(origin, file_id, signer.clone())?;
                        }
                    }
                    Ok(().into())
            })?;
                ...
                project_id, |project_option| -> DispatchResultWithPostInfo {
                    match project_option {
                        ...
                        Some(project) => {
                            ...
                            ensure!(pallet_evercity_filesign::Pallet::<T>::address_is_signer_for_file(project_documentation_file_id, &caller), 
                                Error::<T>::IncorrectProjectSigner);
                            Self::change_project_state(project, caller, &mut event_opt)?;
                            pallet_evercity_filesign::Pallet::<T>::sign_latest_version(origin, project_documentation_file_id)?;
                        }
                    }
                    Ok(().into())
                })?;
                ...
                project_id, |project_to_mutate| -> DispatchResultWithPostInfo {
                    match project_to_mutate  {
                        ...
                        Some(proj) => {
                            ...
                            pallet_evercity_filesign::Pallet::<T>::assign_signer(origin.clone(), proj.annual_reports[len - 1].file_id, signer.clone())?;
                        }
                    }
                    Ok(().into())
             })?;
                ...
                project_id, |project_to_mutate| -> DispatchResultWithPostInfo {
                    match project_to_mutate  {
                        ...
                        Some(proj) => {
                            ...
                            let has_signed = pallet_evercity_filesign::Pallet::<T>::address_has_signed_the_file(proj.annual_reports[len - 1].file_id, &signer);
                            ensure!(!has_signed, Error::<T>::AccountAlreadySigned);
                            proj.annual_reports[len - 1].remove_required_signer((signer.clone(), role));
                            // delete signer in filesign pallet
                            pallet_evercity_filesign::Pallet::<T>::delete_signer(origin.clone(), proj.annual_reports[len - 1].file_id, signer.clone())?;
                        }
                    }
                    Ok(().into())
             })?;
                ...
                project_id, |project_to_mutate| -> DispatchResultWithPostInfo {
                    match project_to_mutate {
                        ...
                        Some(project) => {
                            ...
                            ensure!(pallet_evercity_filesign::Pallet::<T>::address_is_signer_for_file(annual_report_file_id, &caller), 
                                Error::<T>::IncorrectAnnualReportSigner);
                            Self::change_project_annual_report_state(project, caller, &mut event_opt)?;
                            pallet_evercity_filesign::Pallet::<T>::sign_latest_version(origin, 
        ...
        pub fn release_carbon_credits(
            ...
            ensure!(accounts::Pallet::<T>::account_is_cc_project_owner(&project_owner), Error::<T>::AccountNotOwner);
                ...
                project_id, |project_option| -> DispatchResult {
                    match project_option {
                        ...
                        Some(project) => {
                            ...
                            let create_asset_call = pallet_evercity_assets::Call::<T>::create
                                {id: asset_id, admin: new_carbon_credits_holder_source.clone(), max_zombies: MAX_CARBON_CREDITS_ZOMBIES, min_balance};
                            ...
                            let set_metadata_call = pallet_evercity_assets::Call::<T>::set_metadata {id: asset_id, 
                                                    name: meta.name, 
                                                    symbol: meta.symbol, 
                            ...
                            let result = set_metadata_call.dispatch_bypass_filter(origin.clone());
                                ...
                                // destroy if failed
                                let _ = pallet_evercity_assets::Call::<T>::destroy{id: asset_id, zombies_witness: 0};
                            ...
                            let mint_call = pallet_evercity_assets::Call::<T>::mint {id: asset_id, beneficiary: new_carbon_credits_holder_source, amount: cc_amount};
                ...
                project_id, |project_option| -> DispatchResult {
                    match project_option {
                        ...
                        Some(project) => {
                            ...
                            let create_asset_call = pallet_evercity_assets::Call::<T>::create
                                {id: asset_id, admin: new_carbon_credits_holder_source, max_zombies: MAX_CARBON_CREDITS_ZOMBIES, min_balance: Self::u64_to_balance(1)};
                            let create_asset_result = create_asset_call.dispatch_bypass_filter(origin.clone());
                            ensure!(!create_asset_result.is_err(), Error::<T>::ErrorCreatingAsset);
        ...
        pub fn burn_carbon_credits(
            ...
            BurnCertificates::<T>::try_mutate(
                credits_holder.clone(), |certificates| -> DispatchResult {
                    ...
                    let burn_call = pallet_evercity_assets::Call::<T>::burn_self_assets{id: asset_id, amount};
		...
		pub fn create_carbon_credit_lot(
			...
			if let Some(sum) = total_sum {
				ensure!(sum <= pallet_evercity_assets::Pallet::<T>::balance(asset_id, caller.clone()), 
		...
		pub fn buy_carbon_credit_lot_units(
			...
			ensure!(total_price < pallet_evercity_bonds::Pallet::<T>::get_balance(&caller),
                    ...
                    match lots_opt {
                        Some(lots) => {
                            if let Some(index) = lots.iter().position(|item| item==&lot ){
                                ...
                                pallet_evercity_bonds::Pallet::<T>::transfer_everusd(
                                    &caller, 
                                    &seller, 
                                    total_price
                                )?;
        ...
        pub fn external_verify_batch_asset(
            ...
            ensure!(accounts::Pallet::<T>::account_is_manager(&caller), Error::<T>::AccountIncorrectRole);
            BatchAssetRegistry::<T>::try_mutate(batch_id, |batch| -> DispatchResult {
                if let Some(batch) = batch {
                    ...
                    let create_asset_call = 
                        pallet_evercity_assets::Call::<T>::create {id:asset_id, admin: holder.clone(), max_zombies: MAX_CARBON_CREDITS_ZOMBIES, min_balance};
                    let create_asset_result = create_asset_call.dispatch_bypass_filter(origin.clone());
                    ensure!(!create_asset_result.is_err(), Error::<T>::ErrorCreatingAsset);

                    // Set metadata 
                    let set_metadata_call = pallet_evercity_assets::Call::<T>::set_metadata{id: asset_id, 
                                            name: batch.construct_carbon_asset_name(), 
                                            symbol: batch.construct_carbon_asset_symbol(), 
                                            decimals: 0
                                        };
                    ...
                    let mint_call = pallet_evercity_assets::Call::<T>::mint {id: asset_id, beneficiary: holder, amount: cc_amount};
                    let result = mint_call.dispatch_bypass_filter(holder_origin);
                    ensure!(!result.is_err(), {
                        // destroy if failed
                        let _ = pallet_evercity_assets::Call::<T>::destroy {id: asset_id, zombies_witness: 0};
    ...
    impl<T: Config> Pallet<T> {
                ...
                Standard::GOLD_STANDARD => {
                    match project.state {
                        ...
                        project::AUDITOR_SIGN_PENDING => {
                            ensure!(accounts::Pallet::<T>::account_is_cc_auditor(&caller), Error::<T>::AccountNotAuditor);
                        ...
                        project::STANDARD_SIGN_PENDING => {
                            ensure!(accounts::Pallet::<T>::account_is_cc_standard(&caller), Error::<T>::AccountNotStandard);
                        ...
                        project::REGISTRY_SIGN_PENDING => {
                            ensure!(accounts::Pallet::<T>::account_is_cc_registry(&caller), Error::<T>::AccountNotRegistry);
        ...
        pub fn balance(asset_id: AssetId<T>, account_id: T::AccountId) -> T::ABalance {
            pallet_evercity_assets::Pallet::<T>::balance(asset_id, account_id)
        }
        ...
        pub fn transfer_carbon_credits(
            ...
            let new_carbon_credits_holder_source = <T::Lookup as StaticLookup>::unlookup(new_carbon_credits_holder.clone());
            let transfer_call = pallet_evercity_assets::Call::<T>::transfer {id: asset_id, target: new_carbon_credits_holder_source, amount};
